...
def _getPage(page, token):
    ...
    page = requests.post('https://www.hnx.vn/ModuleIssuer/UC_Issuer/ListSearch_Datas', data=data, headers=headers)
...
def _getSecurityInfoFromRow(row):
    return {
        ...
        'start_date': datetime.datetime.strptime(row.xpath('td[4]/text()')[0].strip(), '%d/%m/%Y'),
        # TODO: clarify the meaning of the below 2 fields
        'total_listing_quantity': int(utils.strip(row.xpath('td[5]/text()')[0], ' .,')),
        'total_listing_value': int(utils.strip(row.xpath('td[6]/text()')[0], ' .,')),
    }
